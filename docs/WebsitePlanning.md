...
We are using Leaflet, an open-source JavaScript library designed for building interactive maps, because it is lightweight, fast, and highly customizable. Leaflet allows us to easily display and interact with maps in the browser, providing essential features such as zooming, panning, and adding markers or custom layers. For our project, Leaflet is especially beneficial because it enables us to show user locations, quests and routes on an intuitive map interface which enhances the overall user experience. Its simplicity, combined with strong community support and compatibility with various map providers, makes it a practical and efficient choice for integrating real-time location-based features into our application.
